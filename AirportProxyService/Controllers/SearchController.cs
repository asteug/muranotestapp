...
namespace AirportProxyService.Controllers
{
    public class SearchController : Controller
    {
        ...
        public async Task<ActionResult> SearchDestinations(SearchDestinationModel model)
        {
            ...
            try
            {
                ...
                do
                {
                    if (iteration == 0)
                    {
                        ...
                        allRoutes.AddRange(routesInCityAirports.Result.Where(w => !w.DestAirport.Contains("\\N"))
                            .Select(s => s.DestAirport).ToList());
                    }
                    else if (iteration < 4)
                    {
                        ...
                        var tasksCheckAirplaneRoutes = routes.Where(w => !w.IsChecked).Select(
                            async items =>
                            {
                                foreach (var item in items.Routes)
                                {
                                    ...
                                    routes1.AddRange(routesInCityAirports.Result
                                        .Where(w => !w.DestAirport.Contains("\\N")).Select(s => s.DestAirport));
                                }
                            });

                        await Task.WhenAll(tasksCheckAirplaneRoutes);
                        allRoutes = new HashSet<string>(routes1).ToList();
                        routes.Where(w => w.Iteration <= iteration - 1).Select(s =>
                        {
                            s.IsChecked = true;
                            return s;
                        }).ToList();
                ...
                if (allRoutes.Any(a => a == model.ToAirport))
                {
                    ...
                    {
                        StaticClass.ListOfChain.Reverse();
                        var tasks = StaticClass.ListOfChain.Select(async items =>
                        {
                            var airlinesAccess =
                                await new AirlineApiIntegrationMethods()
                                    .GetAllAirlineByAliasAsync(items.Airline, token);
                            SearchResultAirportExtensions.Add(new RouteExtension()
                            {
                                ...
                                IsWorkingAirline =
                                    airlinesAccess.Result.Any(a => a.Alias == items.Airline && a.IsActive)
                            });
                        });
            ...
            {
                Log.Logger.Error(ex,
                    $"{nameof(SearchAirport)} exception handler\r\n {ex.Message} \r\n Inner exception: {ex.InnerException?.Message}");
